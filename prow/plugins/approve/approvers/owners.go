...
**Once this PR has been reviewed and has the lgtm label**, please ask for approval from {{range $index, $cc := .ap.AssignedCCs}}{{if $index}}, {{end}}{{printf "[%s](https://github.com/%s)" $cc $cc}}{{end}} and additionally assign {{range $index, $cc := .ap.SuggestedCCs}}{{if $index}}, {{end}}{{printf "[%s](https://github.com/%s)" $cc $cc}}{{end}} for approval(**Please ensuring that each of them provides their approval before proceeding**). For more information see [the Code Review Process]({{ .prProcessLink }}).
{{- else -}}
**Once this PR has been reviewed and has the lgtm label**, {{ if eq (len .ap.SuggestedCCs) 1 }}please assign {{else}}please assign {{end}}{{range $index, $cc := .ap.SuggestedCCs}}{{if $index}}, {{end}}{{printf "[%s](https://github.com/%s)" $cc $cc}}{{end}}{{ if eq (len .ap.SuggestedCCs) 1 }} for approval.{{else}} for approval, **ensuring that each of them provides their approval before proceeding**.{{end}} For more information see [the Code Review Process]({{ .prProcessLink }}).
{{- end}}
{{- else -}}
{{- if len .ap.AssignedCCs -}}
**Once this PR has been reviewed and has the lgtm label**, please ask for approval from {{range $index, $cc := .ap.AssignedCCs}}{{if $index}}, {{end}}{{printf "[%s](https://github.com/%s)" $cc $cc}}{{end}}, **ensuring that each of them provides their approval before proceeding**. For more information see [the Code Review Process]({{ .prProcessLink }}).
{{- end}}
{{- end}}
{{- end}}
